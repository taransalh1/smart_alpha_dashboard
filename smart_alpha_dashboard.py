...
try:
    ...
    st.error(f"❌ Failed fetching Alpha list: {e}")
...
try:
    ...
    st.error(f"❌ Failed fetching Binance 24h data: {e}")
    st.code(traceback.format_exc())
    st.stop()

# ----------------------------------------------------------
# MAIN LOOP: BUILD TOKEN DATA
# ----------------------------------------------------------
rows = []
